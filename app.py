...
if go:
    ...
    if not orders:
        ...
        rows, total = pack.schedule(orders, int(cp), teukgam, int(choji))
        st.session_state.result = (rows, total, orders)

if "result" in st.session_state:
    rows, total, orders = st.session_state.result
    st.divider()
    ...
    st.download_button("⬇️ 엑셀(.xlsx) 다운로드", xlsx,
                       file_name=f"지폭스케줄_{lot or 'output'}.xlsx",
                       mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                       width="stretch")
    st.caption("자투리·여유분 배치는 작업자 판단으로 조정될 수 있습니다(초안). 표 수정 후 다시 생성하세요.")
